<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1" import="java.sql.*, java.lang.*"%>
    ...
    <head>
        ...
        <script>
            ...
            function checkDegree() {
                const select = document.getElementById("degree").value;
        ...
        <main class="login-body">
            ...
            <form class="form-default" name="myform" action="student_register.jsp" method="POST">
                ...
                <div class="login-form reg">
                    ...
                    <div class="form-input">
                        <input type="text" name="username" id="username" placeholder="Enter UserName" pattern="[A-Za-z0-9]{6,}" title="Only contain Alphabets & Numbers of min 6 characters are accepted" required>
                    </div>
                    <div class="form-input">
	...
	try
	{
		...
		Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project","root","R@fi4143");
...
%>
    ...
    try {
        ...
        String checkUsernameSql = "SELECT COUNT(*) FROM student WHERE username = '" + username + "'";
        ResultSet rsUsername = stmt.executeQuery(checkUsernameSql);
        rsUsername.next();
        int usernameCount = rsUsername.getInt(1);
        rsUsername.close();

        // Check if the email already exists
        String checkEmailSql = "SELECT COUNT(*) FROM student WHERE email_id = '" + email + "'";
        ResultSet rsEmail = stmt.executeQuery(checkEmailSql);
        rsEmail.next();
        ...
        // Handle the counts and respond to the user
        if (usernameCount > 1 && emailCount > 1) {
            ...
            // Redirect to the next step (inserting the new record)
            String sql = "insert into student (name, username, email_id, password, degree, course, year) values('" + name + "','" + username + "','" + email + "',SHA1('" + password + "'),'" + degree + "','" + course + "','" + year + "')";
