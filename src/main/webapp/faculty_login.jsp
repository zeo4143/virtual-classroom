...
%>
	...
	try
	{
        ...
        String sql = "SELECT * FROM faculty WHERE (username = '" + usernameOrEmail + "' OR email_id = '" + usernameOrEmail + "') AND password = SHA1('" + password + "')";
		...
		if(rs.next())
		{
			...
			if(rs.getInt("is_approved") == 0)
			{
                ...
                session.removeAttribute("Error");
                session.setAttribute("Error","Your Request was rejected by Administrator");
            }
			// if request is accepted by Admin
			else if(rs.getInt("is_approved") == 1)
			{
				session.setAttribute("faculty",rs.getString(2));
				response.sendRedirect("faculty_dashboard.jsp");
			}
		}
		...
		{
			// create a SQL statement
            String sql1 = "select * from faculty WHERE (username = '" + usernameOrEmail + "' OR email_id = '" + usernameOrEmail + "')";
